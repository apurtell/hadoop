...
public class TestDataNodeTcpNoDelay {
  ...
  public void testTcpNoDelayEnabled() throws Exception {
    ...
    // active by default
    testConf.set(CommonConfigurationKeysPublic.HADOOP_RPC_SOCKET_FACTORY_CLASS_DEFAULT_KEY,
  ...
  public void testTcpNoDelayDisabled() throws Exception {
    ...
    try {
      ...
      // we can only check that TCP_NODELAY was disabled on some sockets, since part of the
      // client write path always enables TCP_NODELAY by necessity
  ...
  /**
   * Tests the {@link DataNode#transferBlocks(String, Block[], DatanodeInfo[][], StorageType[][])}
   * path by re-replicating an existing block.
   */
  ...
  private void setTcpNoDelay(Configuration conf, boolean value) {
    conf.setBoolean(DFSConfigKeys.DFS_DATA_TRANSFER_CLIENT_TCPNODELAY_KEY, value);
  ...
  public static class SocketWrapper extends Socket {
    ...
    // TODO: override methods, check whether tcpnodelay has been set when socket is closed
    // this isn't perfect, as we could still send before tcpnodelay is set, but should at least
    // trigger when tcpnodelay is never set at all
